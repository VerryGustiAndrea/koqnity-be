const updateVariation = (updateVariationAction: Function, insertLogs: Function) => {
    return async function post(httpRequest: any) {
        try {
            ...
            const posted = await updateVariationAction({
                ...info,
                source
            });
            let insertLog = await insertLogs({
                type_activity: 'update_variation_inventory',
