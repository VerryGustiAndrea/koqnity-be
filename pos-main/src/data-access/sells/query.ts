...
const query = (conn: any, models: any) => {
    ...
    async function getListSell(data: { search: string; status: string; customer: string; min_price: string; max_price: string; start_date: string; end_date: string }) {
        try {
            const pool = await conn();
            const { search, status, customer, min_price, max_price, start_date, end_date } = data; // deconstruct
            const res = await new Promise((resolve) => {
                ...
                sql += ' ORDER BY sells.created_at DESC';
